...
#
ai_win, ai_lose, same = 0, 0, 0
verbose = 1000

# inp와 hidden 데이터타입(dtype)을 float로 주었다. 그 이유는, nn.LSTM은 float형을 기반으로 연산하기 때문이다.)
fights = 10000
ai_df = None
marcof_df = F.one_hot(torch.randint(3, size = (1,)), num_classes = 3).unsqueeze(dim = 0).float()
array_ai = [[0 for row in range(3)] for col in range(3)]
...
optimizer = optim.Adam(lstm_layer.parameters(), lr = 0.01)
...
for counts in range(fights):
    ...
    loss = mod1.for_rsp_loss(ai, marcof, result)
    #print('---for loss------')
    #print('marcof', marcof)
    #print('ai', ai)
    #print('ai: %s, marcof: %s' %(mod1.numtocard(ai_card), mod1.numtocard(marcof_card)))
    #print('loss', loss)
